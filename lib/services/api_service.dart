...
class ApiService {
  ...
  List<Product> _parseProductList(dynamic data) {
    ...
    return list.map((item) {
      ...
      if (item['photographer'] != null &&
          item['photographer']['image'] != null) {
        item['photographer']['image'] = resolveImageUrl(
          item['photographer']['image'].toString(),
        );
      }
  ...
  Future<double?> getProductPrice(
    ...
    final url = Uri.parse(
      '$baseUrl/products/$productId/price?size=${Uri.encodeComponent(size)}',
    );
    try {
      final response = await http
          .get(
      ...
      if (response.statusCode == 200) {
        ...
        return (data['price'] != null)
            ? (num.tryParse(data['price'].toString())?.toDouble())
            : null;
      }
      return null;
    } catch (_) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['photographer'] != null &&
            data['photographer']['image'] != null) {
          data['photographer']['image'] = resolveImageUrl(
            data['photographer']['image'].toString(),
          );
        }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (decoded['user'] != null) {
          ...
          if (isAdmin) {
            throw Exception(
              'Admin dashboard access restricted to web platform.',
            );
          }
        }
        return decoded;
        ...
        if (order['items'] != null) {
          for (var item in order['items']) {
            if (item['product_image'] != null &&
              ...
              item['product_image'] = resolveImageUrl(
              ...
              item['product']['image_url'] = resolveImageUrl(
                item['product']['image_url'].toString(),
              );
            }
          }
        }
  ...
  Future<List<Product>> fetchFavorites(String token) async {
    ...
    if (response.statusCode == 200) {
      final decoded = json.decode(response.body);
      final List<dynamic> list = (decoded is Map && decoded.containsKey('data'))
          ? decoded['data']
          : (decoded is List ? decoded : []);
      return list.map((item) {
        final productData = item['product'] ?? item;
        if (productData['image_url'] != null) {
          productData['image_url'] = resolveImageUrl(
